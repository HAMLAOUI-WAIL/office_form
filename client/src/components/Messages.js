...
const Messages = (props) => {
  ...
  return props.conservant ? (
      ...
      {messages && conversation && !loading ? (
        <>
          ...
          >
            ...
            />
            <Typography>
              <Link to={"/users/" + props.conservant.username} style={textst}>
                <b color={"white"}>{props.conservant.username}</b>
              </Link>
            </Typography>
          </HorizontalStack>
          <Divider />
          <Box sx={{ height: "calc(100vh - 240px)" }} style={ItemStyle}>
            <Box sx={{ height: "100%" }}>
              <Stack
    ...
    <Stack
      ...
      spacing={2}
    >
      <AiFillMessage size={80} color="white" />
      <Typography variant="h5" color={"white"}>
        OfficeFourm Messenger
      </Typography>
      <Typography color="white">
        Privately message other users on OfficeFourm
      </Typography>
    </Stack>
  );
};
